...
# v6.2：每工位樣本表新增 v/i/w 三欄（PW3335 電力）
...
def insert_sample(
    ...
    v6.2：新增 v / i / w 三個 optional 參數，給 PW3335 用。
...
def _row_to_dict(r: sqlite3.Row, station: str) -> Dict[str, Any]:
    ...
    # v6.2：PW3335 欄位（舊 DB 補欄前會跳 KeyError；用 try 包起來向後相容）
...
def _ensure_power_columns(c: sqlite3.Connection, station: str) -> None:
    """v6.2：確保 samples 表有 v / i / w 三欄。
    對 v6.2 之後新建的 DB：SCHEMA_SAMPLES 內已含這三欄，no-op。
